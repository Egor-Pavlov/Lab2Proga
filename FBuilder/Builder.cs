...
namespace FBuilder
{
    public class Builder
    {
        ...
        public bool Init(string FilePath, Form form)
        {
            this.form = form;
        ...
        private int Build(int level = 0, ToolStripDropDownButton root = null, int index = 0)
        {
            ...
            for (; i < Lines.Length; i++)
            {
                ...
                control.Size = new System.Drawing.Size(100, 22);
                ...
                {
                    return -1;//возврат -1 если статус записан некорректно
                }

                //Добавляем вызываемый метод
                if (str[3].Trim() == "FirstMethod")
                    control.Click += FirstMethod;

                else if (str[3].Trim() == "SecondMethod")
                    control.Click += SecondMethod;

                else if (str[3].Trim() == "ThirdMethod")
                    control.Click += ThirdMethod;
